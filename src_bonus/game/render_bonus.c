...
/*   Updated: 2026/02/10 00:10:30 by ntome            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_draw(t_mlx *mlx, t_drawing *draw, t_ray *ray)
{
	...
	if (ray->side == 1)
	{
		...
		if (ray->ray_dir.y > 0)
			draw->texture = mlx->textures.we_wall;
		else
			draw->texture = mlx->textures.ea_wall;
	}
	else
	{
		draw->wall_x = mlx->player.pos.y + ray->perp_wall_dist * ray->ray_dir.y;
		if (ray->ray_dir.x > 0)
			draw->texture = mlx->textures.so_wall;
		else
			draw->texture = mlx->textures.no_wall;
	}
